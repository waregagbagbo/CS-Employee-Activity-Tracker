...
import { FaUsers, FaFileAlt, FaCalendarCheck, FaChartLine, FaPlus, FaArrowRight } from "react-icons/fa";
...
export default function Dashboard() {
  ...
  const fetchCounts = useCallback(async () => {
    try {
      ...
      setCounts({
        ...
        attendance: empRes.data.count === 94 ? 100 : 90, // example
      });
    } catch (err) { console.error("Sync Error", err); }
    finally { setLoading(false); }
  }, []);

  // Fetch shifts + attendance
  ...
  }, [fetchCounts, fetchShiftAttendance]);
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-10 space-y-8 pb-32">
          ...
          <section className="bg-black p-8 md:p-12 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <p className="text-gray-400 mt-2 font-medium max-w-md">System synced. Deployment nodes reporting operational stability.</p>
            </div>
          </section>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <div className="xl:col-span-8 space-y-8">
              ...
              {/* Attendance Clock + Today Summary */}
              <div className="bg-white rounded-[2.5rem] p-4 border border-gray-100 shadow-sm">
                <AttendanceClock userType={userType} attendanceToday={todayAttendance} availableShifts={availableShifts} />
              </div>
                ...
                {availableShifts.length > 0 ? (
                  <ul className="space-y-4">
                    {availableShifts.map(shift => (
                      <li key={shift.id} className="flex justify-between items-center bg-gray-50 rounded-2xl p-4 border border-gray-100">
                        <div>
                        ...
                        <span className={`px-2 py-1 text-[9px] font-black rounded-full uppercase ${shift.shift_status === 'Scheduled' ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600 animate-pulse'}`}>{shift.shift_status}</span>
                      </li>
                    ))}
                  </ul>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {metrics.map((m,i) => (
                  <div key={i} onClick={() => navigate(m.route)} className="bg-white p-5 rounded-3xl border border-gray-100 hover:border-[#FFCC00] transition-all cursor-pointer group">
                    <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-black mb-4 group-hover:bg-[#FFCC00] transition-colors">{m.icon}</div>
