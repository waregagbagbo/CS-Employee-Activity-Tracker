  ...
  const handleClockAction = async () => {
    ...
    try {
      ...
      if (attendanceStatus?.is_clocked_in) {
        ...
        if (!selectedShift) {
          setError("Please select a shift before clocking in.");
          return;
        }
        result = await AttendanceService.clockIn({ shift_id: selectedShift });
      }
    ...
    <div className="flex bg-[#F9FAFB] h-screen w-full font-sans text-black overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-10 space-y-8 pb-32">
          ...
          {/* ERROR BANNER */}
          {error && (
            <div className="bg-rose-50 border border-rose-200 text-rose-600 p-5 rounded-2xl flex items-start gap-3 gap">
