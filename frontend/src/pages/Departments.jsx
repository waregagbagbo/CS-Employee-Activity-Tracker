...
export function Departments() {
  ...
  useEffect(() => {
    const filtered = departments.filter((dept) => {
      ...
      const deptHead = dept.head_node.username || "";

      return title.toLowerCase().includes(searchTerm.toLowerCase()) ||
          desc.toLowerCase().includes(searchTerm.toLowerCase()) ||
          deptHead.toLowerCase().includes(searchTerm.toLowerCase());
    });
      ...
      <div className="flex bg-[#F9FAFB] min-h-screen font-sans">
        ...
        <div className="flex-1 p-8 lg:p-12">
          ...
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            ...
            {canManage ? (
                ...
                <div className="bg-gray-100 border border-gray-200 px-6 py-3 rounded-2xl flex items-center gap-3">
                  ...
                  <span
                      className="text-[10px] font-black uppercase tracking-widest text-gray-400">Read-Only Directory</span>
                </div>
            )}
          </header>
          ...
          <div className="relative mb-10 group">
            <FaSearch
                className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#FFCC00] transition-colors"/>
            <input
                type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredDepartments.map((dept) => (
                ...
                >
                  ...
                  <FaBuilding
                      className="absolute -right-4 -bottom-4 text-gray-50 text-8xl group-hover:text-[#FFCC00]/10 transition-colors"/>
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-xs text-black-400 font-medium line-clamp-2 mb-8 normal tracking-tighter leading-relaxed">
                    ...
                    <div className="flex items-center gap-3 mb-8 p-3 bg-gray-50 rounded-xl border border-gray-100">
                      <div
                          className="w-8 h-8 bg-[#FFCC00] rounded-lg flex items-center justify-center text-black font-black text-xs">
                        <FaUserTie/>
                      </div>
                      <div>
                        <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Head of
                          Department</p>
                        <p className="text-[10px] font-black text-black uppercase">{dept.head_node || "UNASSIGNED"}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 pt-4 border-t border-gray-50">
                      ...
                      {canManage ? (
                          <>
                            ...
                            <button
                                className="p-3 bg-white border border-gray-200 text-gray-400 hover:text-red-600 hover:border-red-600 rounded-xl transition-all">
                              <FaTrash/>
                            </button>
                          </>
                      ) : (
                          <div
                              className="p-3 text-gray-200 flex items-center justify-center bg-gray-50 rounded-xl border border-gray-100">
                            <FaLock size={12}/>
                          </div>
                      )}
                    </div>
          ...
          {filteredDepartments.length === 0 && (
              <div className="text-center py-24 bg-white rounded-[3rem] border border-dashed border-gray-200 mt-10">
                ...
                <p className="text-gray-400 font-black uppercase tracking-[0.3em] text-xs">No matching nodes found in
                  directory</p>
              </div>
          )}
        </div>
